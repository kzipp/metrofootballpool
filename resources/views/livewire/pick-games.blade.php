        ...
        <div wire:key="{{ $game->id }}" class="flex items-center justify-between mb-4 p-4">
            <div class="flex-1 flex justify-start space-x-4">
                <button wire:click="selectWinner({{ $game->id }}, '{{ $game->away_team }}')"
                    class="px-4 py-2 {{ isset($picks[$game->id]['winner']) && $picks[$game->id]['winner'] === $game->away_team ? 'bg-dolphins-green border-dolphins-green text-white' : 'hover:bg-gray-100' }} cursor-pointer">
                ...
                <button wire:click="selectWinner({{ $game->id }}, '{{ $game->home_team }}')"
                    ...
                    class="px-4 py-2 {{ isset($picks[$game->id]['winner']) && $picks[$game->id]['winner'] === $game->home_team ? 'bg-dolphins-green border-dolphins-green text-white' : 'hover:bg-gray-100' }} cursor-pointer">
